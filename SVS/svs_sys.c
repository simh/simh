...
const char *opname_short_bemsh [64] = {
	...
	"уи",	"уим",	"счи",	"счим", "уии",	"сли",  "счпс",	"э47",
...
const char *opname_short_madlen [64] = {
	...
	"rte",	"yta",	"atf",	"fta", 	"e+n", 	"e-n", 	"asn", 	"ntr",
	...
	UNIT *uptr, int32 sw)
{
	...
	if (sw & SWMASK ('M')) {			/* symbolic decode? */
		if (sw & SIM_SW_STOP && addr == svsPC && !(RUU & RUU_RIGHT_INSTR))
			fprintf (of, "-> ");
	...
	for (;;) {
		...
		case '*':		/* instruction */
			if (addr < 010)
				...
				t_mem data =  SET_TAG (word, TAG_INSN);
				memory [addr] = data;
			}
			++addr;
			break;
